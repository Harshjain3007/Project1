...
const createBlog = async function (req, res) {
  try {
    ...
    if (!isValid(req.body.title))
      return res.status(400).send({ staus: false, msg: "Please enter Title" });
    if (!isValid(req.body.body))
      return res
        .status(400)
        .send({ status: false, msg: "Please enter Blog body" });
    if (!isValid(req.body.category))
      return res
        .status(400)
        .send({ status: false, msg: "Please enter Blog category" });
    ...
    if (!req.body.authorId)
      return res
        .status(400)
        .send({ status: false, msg: "AuthorId is mandatory" });
    if (req.body.authorId != req.decodeToken.author_Id)
      return res
        .status(403)
        .send({
          status: false,
          msg: "Wrong authorId / you can create only your blog",
        });
    ...
    for (let i = 0; i < category.length; i++) {
      if (!isValid(category[i]))
        return res
          ...
          .send({
            status: false,
            msg: "You can't enter blank space in the category section",
          });
    }
    if (req.body.isPublished == true) {
      const saveData2 = await blogModel.create({
...
const getBlogDetail = async function (req, res) {
  try {
    ...
    const data = await blogModel.find({
      $and: [{ isPublished: true, isDeleted: false, ...querydata }],
    });
    if (data.length == 0)
    ...
    if (title) {
      if (!isValid(title))
        return res
          ...
          .send({
            status: false,
            msg: "Only blank space is not acceptable, You should enter some data in title part or remove the title part",
          });
    }
    if (body) {
      if (!isValid(body))
        return res
          ...
          .send({
            status: false,
            msg: "Only blank space is not acceptable, You should ente some data in Body part or remove the Body part",
          });
    }
    if (tags) {
      if (!isValid(tags))
        return res
          ...
          .send({
            status: false,
            msg: "Only blank space is not acceptable, You should ente some data in Tags part or remove the Tags part",
          });
    }
    if (subcategory) {
      if (!isValid(subcategory))
        return res
          ...
          .send({
            status: false,
            msg: "Only blank space is not acceptable, You should ente some data in subcategory part or remove the subcategory part",
          });
    }
    const blogId = req.params.blogId;
...
const deletequery = async function (req, res) {
  try {
    ...
    if (Object.keys(data).length == 0)
      return res
        .status(400)
        .send({ status: false, msg: "Please enter any key" });
    if (req.query.authorId)
      return res
        .status(403)
        .send({
          status: false,
          msg: "You can't delete any other author's blog / (if you want to delete your blog, You don't need to enter authorId)",
        });
    ...
    if (finalData.modifiedCount == 0)
      return res
        .status(404)
        .send({ status: false, msg: "No such blog available" });
